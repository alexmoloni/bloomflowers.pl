...
function amAddToCartBtn( $options = null ) {
    ...
    <div class="am-cart-btn <?= $buy_now_btn ? 'is-buy-now-btn' : '' ?>">
		<?php if ( $product ): ?>
			<?php if ( $buy_now_btn ): ?>
                <a data-comp-products="" data-checkout-url="<?= wc_get_checkout_url() ?>" <?= $has_variations ? 'data-variation-id="' . $initial_variation . '"' : '' ?> data-product-id="<?= $product->get_id() ?>" class="btn-cta-2 btn-small btn buy-now-btn js-buy-now-btn js-ajax-add-to-cart" href="<?= wc_get_checkout_url() ?>"><?= __( 'Kup teraz', 'alexmoloni' ) ?></a>
			<?php else: ?>
